...
public class DragonCreator implements Serializable {
    ...
    public Dragon createINSave(Integer id, String name, float coordX,  Float coordY,  String creationDate,
                                  int age,String description, Integer wingspan , DragonCharacter character,
                                  Integer depth, Float numberOfTreasures, String username) {
        Dragon dragon = new Dragon();
        if (id <= 0) {
            System.out.println("{ERROR}: ЗНАЧЕНИЕ id НЕ ДОЛЖНО БЫТЬ МЕНЬШЕ НУЛЯ");
            System.exit(1);
        } else
            dragon.setId(id);
        dragon.setName(name);
